...
function DetailBlock({ title, items }: { title: string; items: string[] }) {
  return (
    <section className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-6 md:p-8">
      ...
      <ul className="mt-4 space-y-4 text-sm leading-8 text-[rgb(var(--muted))] md:text-base">
        {items.map((item) => (
          <li key={item}>{item}</li>
        ))}
      </ul>
    ...
    <div className="mx-auto min-h-screen max-w-7xl px-4 pb-12 pt-8 md:px-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          <Link
            href="/projects"
            className="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm text-[rgb(var(--fg))]"
          >
            All Projects
          </Link>
          <a
      ...
      <section className="section-shell relative overflow-hidden px-6 py-10 md:px-10 md:py-14">
        ...
        <Image
          src={project.image}
          alt={`${project.title} engineering diagram`}
          fill
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-base leading-8 text-white/82 md:text-lg">{project.subtitle}</p>
            <div className="mt-6 flex flex-wrap gap-4 text-sm text-white/80">
              <span>{project.organization}</span>
              <span>{project.role}</span>
              <span>{project.period}</span>
            </div>
            <p className="mt-8 max-w-3xl text-sm leading-8 text-white/88 md:text-base">{project.overview}</p>
          </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <DetailBlock title="Engineering Challenges" items={project.engineeringChallenges} />
        <DetailBlock title="Design Approach" items={project.designApproach} />
        <DetailBlock title="Validation / Testing" items={project.validationTesting} />
        <DetailBlock title="Results / Impact" items={project.resultsImpact} />
      </div>

      <div className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <DetailBlock title="Next Steps" items={project.nextSteps} />
        <section className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-6 md:p-8">
          <div className="text-xs uppercase tracking-[0.28em] text-[rgb(var(--accent))]">Project Media</div>
          <div className="mt-4 aspect-video rounded-[1.5rem] border border-dashed border-white/15 bg-black/20 p-5">
            <div className="flex h-full items-center justify-center text-center text-sm leading-7 text-[rgb(var(--muted))]">
